...
class ScaledMultivariateDiagonalNormal(TorchDistributionWrapper):
    ...
    def __init__(self, loc, scale_diag, scaling_constant=1, reinterpreted_batch_ndims=1):
        dist = Independent(TorchNormal(loc, scale_diag), reinterpreted_batch_ndims=reinterpreted_batch_ndims)
        super().__init__(dist)
    ...
    def rsample(self):
        ...
        value = torch.clamp(value, -self.scaling_constant + self.eps, self.scaling_constant - self.eps)
        value = value / self.scaling_constant
...
class TanhNormal(Distribution):
    ...
    def _log_prob_from_pre_tanh(self, pre_tanh_value):
        ...
        correction = -2. * (torch.tensor(np.log([2.]), dtype=torch.float32, device=self.device) - pre_tanh_value -
